      ...
      <nav>
        {pins.map((pin) => (
          <div>
            ...
            {
              save ? <button onClick = {async(e)=> {
              e.preventDefault();
              const SaveRes = await dispatch(pinsAction.unSavePinThunk(pin, board))}
              }>Saved</button> : 
              <button onClick={ async(e)=> {
              e.preventDefault();
              const SaveRes = await dispatch(pinsAction.savePinThunk(pin, board))
            }
            }>Save</button>
            }
          </div>
